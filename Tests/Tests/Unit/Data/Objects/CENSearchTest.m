...
- (BOOL)hasMockedObjectsInTestCaseWithName:(NSString *)name {
    ...
    return ([name rangeOfString:@"testAugmentation"].location != NSNotFound ||
            [name rangeOfString:@"testRestoreState"].location != NSNotFound ||
            [name rangeOfString:@"testSearch"].location != NSNotFound ||
            [name rangeOfString:@"testNext"].location != NSNotFound);
}
...
- (void)testConstructor_ShouldCreateWithPlugins_WhenSenderOrEventNamePassed {
    ...
    NSDictionary *expectedConfiguration = @{ @"sender": user.uuid, @"event": @"test-event" };
    CENChat *chat = [self publicChatWithChatEngine:self.client];
    ...
    OCMExpect([self.client registerPlugin:[CENSearchFilterPlugin class] withIdentifier:[OCMArg any]
                            configuration:expectedConfiguration forObject:[OCMArg any] firstInList:YES
...
- (void)testNext_ShouldNotSearch_WhenThereIsNoMoreData {
    ...
    [self waitForObject:searchMock recordedInvocationNotCall:recorded afterBlock:^{
        search.next();
    }];
}
