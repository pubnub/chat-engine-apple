 ...
 *
 * @discussion Useful for storing things like the name and description. Call
 * \b [CENChat updateMeta:] to update the remote information.
 *
 ...
 *
 * @discussion If user manually disconnects via \b [CENChatEngine disconnectUser], the chat is put
 * to 'sleep'. If a connection is reestablished via \b [CENChatEngine reconnectUser], sleeping chats
 ...
 *
 * // Create new chat which will be used to enable participants state restore.
 * CENChat *chat = [self.client createChatWithName:nil private:NO autoConnect:YES metaData:nil];
 *
 * // Enable participants state restore.
 ...
 *
 * // Enable participants state restore.
 * [chat restoreStateForChat:stateChat];
 * @endcode
 *
 ...
 */
- (void)setState:(nullable NSDictionary *)state;
