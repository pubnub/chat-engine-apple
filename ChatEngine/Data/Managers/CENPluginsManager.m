...
- (void)registerExtensionFromPlugin:(CEPPlugin *)plugin withObject:(CENObject *)object {
    ...
    CEPExtension *extension = [cls extensionWithIdentifier:identifier configuration:configuration];
...
- (void)extensionForObject:(CENObject *)object
            withIdentifier:(NSString *)identifier
                   context:(void(^)(id extension))block {
    
    if (![CEPPlugin isValidIdentifier:identifier] || ![CEPPlugin isValidObject:object] || !block) {
        [NSException raise:NSInvalidArgumentException
                    format:@"Parameters is empty or has unexpected data type."];
    }
    
    dispatch_async(self.resourceAccessQueue, ^{
        block(self.extensions[object.identifier][identifier]);
    });
}
...
- (void)registerMiddlewaresFromPlugin:(CEPPlugin *)plugin
    ...
    for (NSString *location in CEPMiddleware.locations) {
        ...
        CEPMiddleware *middleware = [cls middlewareWithIdentifier:plugin.identifier
