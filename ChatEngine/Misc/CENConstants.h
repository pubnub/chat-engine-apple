...
/**
 * @brief Client library version number
 */
static NSString * const kCENLibraryVersion = @"0.9.2";

/**
 * @brief Information about SDK codebase
 */
...
/**
 * @brief \b PubNub Functions entry point.
 */
...
/**
 * @brief Default global chat channel (or namespace to which belongs all \b {chats CENChat}).
 */
static NSString * const kCENDefaultGlobalChannel = @"chat-engine";

/**
 * @brief Whether \b {CENChatEngine} should synchronize changes of \b {chats CENChat}
 * list between \b {local user CENMe} devices or not.
 */
static BOOL const kCENDefaultShouldSynchronizeSession = NO;

/**
 * @brief Whether \b {CENChatEngine} should create and throw exceptions when any error
 * emitted.
 */
#if DEBUG
...
/**
 * @brief Whether \b {CENChatEngine} should synchronize meta information for new chats or
 * not.
 */
